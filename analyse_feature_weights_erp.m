...
for sbj=1:ANALYSIS.nsbj
    ...
    % feature weights are not just | average feature weigths | (!!!)
    
    if ANALYSIS.stmode~=2
...
% comparisons (=features/channels) and once without (p < krit)

% T-test for all single analysis time steps________________________________
for p_corr=1:2 % run for corrected/uncorrected
    
    if p_corr==1
        p_crit=ANALYSIS.pstats;
    elseif p_corr==2
        p_crit=ANALYSIS.pstats/size(FW_ANALYSIS.ALL_Z,3);  
    end
    ...
    for steps=1:size(FW_ANALYSIS.fw_analyse,2)
        ...
        for channel=1:size(FW_ANALYSIS.ALL_Z,3)
            ...
            [h,p]=ttest(temp_z,0,p_crit,'right');
            h_matrix_z(channel,1)=h;
            p_matrix_z(channel,1)=p;
...
%% DISPLAY - HEAT MAP FOR EACH RELEVANT STEP: Z-STANDARDISED ABSOLUTE FEAUTRE WEIGHTS (FOR STATS)
%__________________________________________________________________________
