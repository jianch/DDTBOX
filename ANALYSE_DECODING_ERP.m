...
case 3 % Strong FWER Control Permutation Test
    ...
    for na = 1:size(ANALYSIS.RES.mean_subj_acc,1) % analysis
        ...
        [ANALYSIS.RES.h_ttest(na, :), ANALYSIS.RES.p_ttest(na,:)] = MCC_blaire_karniski_permtest(real_decoding_scores, perm_decoding_scores, 'alpha', ANALYSIS.pstats, 'iterations', ANALYSIS.nIterations);
    end % of for na loop
...
case 4 % Cluster-Based Permutation Test
    ...
    for na = 1:size(ANALYSIS.RES.mean_subj_acc,1) % analysis
        ...
        [ANALYSIS.RES.h_ttest(na, :)] = MCC_cluster_permtest(real_decoding_scores, perm_decoding_scores,  'alpha', ANALYSIS.pstats, 'iterations', ANALYSIS.nIterations, 'clusteringalpha', ANALYSIS.cluster_test_alpha);
    end % of for na loop
    clear real_decoding_scores
    clear perm_decoding_scores
%__________________________________________________________________________    
...
case 5 % KTMS Generalised FWER Control Using Permutation Testing
    ...
    for na = 1:size(ANALYSIS.RES.mean_subj_acc,1) % analysis
        ...
        [ANALYSIS.RES.h_ttest(na, :)] = MCC_KTMS_GFWER(real_decoding_scores, perm_decoding_scores, 'alpha', ANALYSIS.pstats, 'iterations', ANALYSIS.nIterations, 'KTMS_u', ANALYSIS.KTMS_u);
    end % of for na loop
    clear real_decoding_scores
    clear perm_decoding_scores
