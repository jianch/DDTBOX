...
STUDY.stmode = options.stmode; % SPACETIME mode (1=spatial / 2=temporal / 3=spatio-temporal)
STUDY.avmode = options.avmode; % AVERAGE mode (1=no averaging; single-trial / 2=run average) 
STUDY.analysis_mode = options.analysis_mode;% ANALYSIS mode (1=SVM classification, 2=LDA classification, 3=SVR increments, 4=SVR continous)

STUDY.window_width_ms = options.window_width_ms; % width of sliding window in ms
STUDY.step_width_ms = options.step_width_ms; % step size with which sliding window is moved through the trial

STUDY.perm_test = options.perm_test; % run the permutation-decoding? 0=no / 1=yes
STUDY.perm_disp = options.perm_disp; % display the permutation results in figure? 0=no / 1=yes
STUDY.display_on = options.display_on; % 1=figure displayed, 0=no figure

STUDY.rt_match = options.rt_match; % Use RT-matching algorithm to select trials? 0=no / 1=yes
STUDY.feat_weights_mode = options.feat_weights_mode; % Extract feature weights? 0=no / 1=yes
